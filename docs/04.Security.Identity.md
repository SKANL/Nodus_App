...
1.  **Launch:** Judge opens App.
2.  **Event QR:** The Judge scans the "Master Event QR" displayed on the main screen/projector.
    - **Content (Encrypted):** `AES_Encrypt(Raw_Config, User_Password_Hash)`
3.  **Bootstrap:** Judge enters "Event Password" (provided orally by organizer).
    - App decrypts payload -> Gets `Shared_AES_Key`, `Server_UUID`, & `Event_ID`.
4.  **Auth Challenge:** The App now knows _how_ to talk to the server. Without the password, the QR is useless.
