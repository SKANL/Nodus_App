...
public class BleClientService : IBleClientService, IDisposable
{
    ...
    public async Task<Result> StartScanningForServerAsync(CancellationToken ct = default)
    {
        ...
        {
            _logger.LogWarning("Bluetooth permission not granted");
            return Result.Failure("Bluetooth permission not granted");
        }
    ...
    public async Task<Result> SendVoteAsync(Nodus.Shared.Models.Vote vote, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            return await SendPacketAsync(packet, vote, ct);
        }
        catch (Exception ex)
        {
    ...
    {
        var aes = await SecureStorage.Default.GetAsync(NodusConstants.KEY_SHARED_AES);
        var priv = await SecureStorage.Default.GetAsync(NodusConstants.KEY_PRIVATE_KEY);
        var pub = await SecureStorage.Default.GetAsync(NodusConstants.KEY_PUBLIC_KEY);
        var name = await SecureStorage.Default.GetAsync(NodusConstants.KEY_JUDGE_NAME);
        
        return (aes ?? "", priv ?? "", pub ?? "", name ?? "Unknown");
    }

    /// <summary>
    /// Retry logic with exponential backoff for transient BLE failures.
    /// </summary>
        ...
        CancellationToken ct)
    {
        ...
        for (int attempt = 0; attempt <= maxRetries; attempt++)
        {
            ...
            if (result.IsSuccess)
            {
                if (attempt > 0)
                {
                    _logger.LogInformation("Operation succeeded after {Attempts} retries", attempt);
                }
                return result;
            }
            ...
            if (attempt < maxRetries)
            {
                _logger.LogWarning("Attempt {Attempt}/{MaxRetries} failed: {Error}. Retrying in {Delay}ms...", 
                    attempt + 1, maxRetries + 1, result.Error, delay.TotalMilliseconds);
            ...
            {
                _logger.LogError("Operation failed after {MaxRetries} retries: {Error}", 
